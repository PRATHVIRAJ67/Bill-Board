...
// .wrangler/tmp/bundle-TglnEl/checked-fetch.js
...
// .wrangler/tmp/bundle-TglnEl/strip-cf-connecting-ip-header.js
...
app.get("/api", (c) => {
  return c.json({ status: "ok", service: "The Board Secure Backend API", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
});
app.get("/api/spots", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data, error } = await supabase.from("spots").select("*").order("id", { ascending: true });
    if (error) {
...
var handleCreateOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    const body = await c.req.json();
    let amountInPaise = 0;
    if (body.amount) {
...
app.post("/api/spots/claim", async (c) => {
  try {
    ...
    const supabase = getSupabaseClient(c.env);
    const { data: spotData, error: spotError } = await supabase.from("spots").update({
      handle: handle.trim(),
...
// .wrangler/tmp/bundle-TglnEl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TglnEl/middleware-loader.entry.ts
